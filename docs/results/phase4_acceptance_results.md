...
- 后端：`uvicorn src.loopse.main:app --reload --port 8000`（本地 Mock 模式）
- 前端：`npm run dev`（http://localhost:5173）或 `npm run build`
- 日期：2026-07-12
- 知识库：公开 RFC/IANA 文档 + 本地 JSON 检索索引；ChromaDB 可选
- pytest 命令：`.venv/Scripts/python.exe -m pytest tests/unit/ tests/test_student_profile_schema.py -v`
...
tests/unit/test_llm_client.py                 14 passed
tests/unit/test_resource_generator.py          5 passed
tests/unit/test_resource_generator_all_types.py 13 passed
tests/test_student_profile_schema.py          17 passed
总计                                          59 passed, 0 failed
```
