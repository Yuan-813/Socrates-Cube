<template>
  <div class="resource-card code-card bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow">
    <div class="flex items-start gap-3">
      <div class="shrink-0 w-9 h-9 rounded-lg bg-emerald-50 flex items-center justify-center text-emerald-500">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-1">
          <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600">代码示例</span>
          <span class="text-xs px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">
            {{ resource.metadata?.language ?? 'python' }}
          </span>
        </div>
    ...
    <button
      class="mt-2 w-full text-xs text-emerald-500 hover:text-emerald-600 font-medium"
